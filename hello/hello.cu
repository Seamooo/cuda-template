...
namespace hello {
	class op_functor {
		...
		__host__ __device__ auto operator()(const float& x, const float& y) const -> float {
			return a * x + y;
		}
	};
	...
	void hello() {
		...
		thrust::transform(d_x.begin(), d_x.end(), d_y.begin(), d_y.begin(), op_functor(A));
		thrust::host_vector<float> res = d_y;
		std::cout << A << " * " << X << " + " << Y << " * " << "[" << NELEMS << "]:\n";
		std::cout << "[";
